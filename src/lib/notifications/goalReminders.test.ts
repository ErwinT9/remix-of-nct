...
const base = (o: Partial<ReminderItem>): ReminderItem => ({
  id: "1", title: "Walk", kind: "goal", start_date: dayKey(now), end_date: null,
  time_of_day: "morning", repeat_type: "daily", repeat_days: [], is_paused: false,
  reminder_enabled: true, reminder_time: "17:00", ...o,
});

test("off = nothing", () => expect(planReminders([base({ reminder_enabled: false })], now)).toHaveLength(0));
...
test("one-off before start not fired", () =>
  expect(planReminders([base({ repeat_type: "none", start_date: shiftDay(dayKey(now), 3) })], now)).toHaveLength(1));
...
test("paused skipped", () => expect(planReminders([base({ is_paused: true })], now)).toHaveLength(0));
